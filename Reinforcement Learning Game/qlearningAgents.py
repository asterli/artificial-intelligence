...
class QLearningAgent(ReinforcementAgent):
    ...
    def computeActionFromQValues(self, state):
        ...
        if not legalActions: #return None if there are no legal actions
            return None
        Q = self.computeValueFromQValues(state) # compute the max q value using computerValueFromQValues
        A = []
        for action in legalActions:
            if self.getQValue(state, action) == Q: # if the current action is the best action we can take, add to A
                A.append(action)
        if A:
